...
class ConfiModel:
    ...
    def est_params(self):
        if not self.exp_config['split_data_f'] == 1:
            params = ['sig_Vrh',  'sig_Vrl'   ,'sig_A'      ,'kC1'           ,'mC1'    ]
            bounds = [(1e-3, 5),  (1e-1, 20)  ,(1e-1, 20)   ,(1e-8, 50)      ,(-10, 10)]  # bounds = []
            p_bounds=[(1e-3, 2),  (2, 10)     ,(2, 15)      ,(1e-8, 20)      ,(-5, 5)  ]  # plausible bounds [PLB, PUB], more narrow than the bounds
        else:
            params = [ 'sig_V'   ,'sig_A'      ,'kC1'           ,'mC1'    ]
            bounds = [ (1e-1, 20)  ,(1e-1, 20)   ,(1e-8, 50)      ,(-10, 10)]  # bounds = []
            p_bounds=[ (2, 10)     ,(2, 15)      ,(1e-8, 20)      ,(-5, 5)  ]  # plausible bounds [PLB, PUB], more narrow than the bounds

        if self.exp_config['eccent_flag'] == 1:
            params.append('w_vis')
            bounds.append((1e-4, 0.5))  
            p_bounds.append((1e-4, 0.1))
        ...
        if self.model_config['Causal_readout'] == 'Bay' or self.model_config['perc_readout'] == 'Bay':
            if self.exp_config['share_prior_f']==1:
                ...
                p_bounds.extend([(5, 30)  ,(5, 30)  ,(0.1, 0.9)       ,(0.1, 0.9)])

        if self.model_config['CI_readout'] == 'BayInt' and self.exp_config['rng_flag'] == 0:
            params.append('rng')
            bounds.append((1e-8, 0.8)) 
            p_bounds.append((0.1, 0.3)) 
        elif self.model_config['CI_readout'] == 'BayInt' and self.exp_config['rng_flag'] == 1:
        ...
        if self.exp_config['continous_flag'] == 0:
            ...
            bounds.extend([(1e-8, 2),  (1e-8, 2), (1e-8, 2)])  
            p_bounds.extend([(1e-8, 1), (1e-8, 1), (1e-8, 1)])
    ...
    def run_simulation(self, inputs, trialConds=None, interimOutput=False):
        ...
        ################ Causal Inference ######################
        ########################################################
        if self.model_config['Causal_readout'] == 'Bay':
            if self.model_config['perc_readout']!='Bay':
        ...
        elif self.model_config['Causal_readout'] == 'NonBay' :
