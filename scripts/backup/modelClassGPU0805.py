...
class ConfiModel:
    ...
    def load_config(self):
        ...
        """
        config_df = pd.read_csv('m_config0805.csv')  # Read full config file
    ...
    def est_params(self):
        ...
        if self.config['Causal_readout'] == 'Bay' and self.config['CI_readout'] == 'BayIntFlex':
            params = ['sig_Vrh',  'sig_Vrl',   'sig_A',    'sig_P', 'p_com',        'sig_rs',      'sig_rconf',      'sig_rc',      'rng',         'kC1',     'mC1']
            bounds = [(1e-3, 5),  (1e-1, 20),  (1e-1, 20), (1, 60), (1e-4, 1-1e-4),  (1e-10, 1),    (1e-10, 1),    (1e-10, 1),   (1e-8, 0.8),     (1e-8, 50),   (-10, 10)]  #bounds = []
            p_bounds=[(1e-3, 2),  (2, 10),     (2, 15),    (5, 30), (0.3, 0.7),      (1e-10, 0.5), (1e-10, 0.5), (1e-10, 0.5),    (0.1, 0.3),    (1e-8, 20),    (-5, 5)] # plausible bounds [PLB, PUB], more narrow than the bounds
        elif self.config['Causal_readout'] == 'Bay' and self.config['CI_readout'] == 'BayIntFix':
            params = ['sig_Vrh',  'sig_Vrl',   'sig_A',    'sig_P', 'p_com',        'sig_rs',      'sig_rconf',      'sig_rc',       'a',             'b',       'kC1',     'mC1']
            bounds = [(1e-3, 5),  (1e-1, 20),  (1e-1, 20), (1, 60), (1e-4, 1-1e-4),  (1e-10, 1),    (1e-10, 1),    (1e-10, 1),   (1e-8, 50),   (1e-8, 10),   (1e-8, 50),   (-10, 10)]  #bounds = []
            p_bounds=[(1e-3, 2),  (2, 10),     (2, 15),    (5, 30), (0.3, 0.7),      (1e-10, 0.5), (1e-10, 0.5), (1e-10, 0.5),   (1e-8, 50),    (1e-8, 5),   (1e-8, 20),    (-5, 5)] # plausible bounds [PLB, PUB], more narrow than the bounds

        else:
            params = ['sig_Vrh',  'sig_Vrl',     'sig_A',    'sig_rs',      'sig_rconf',      'sig_rc',       'a',             'b',    'k_emp']
            bounds = [(1e-3, 5),  (1e-1, 20),  (1e-1, 20),  (1e-10, 1),    (1e-10, 1),    (1e-10, 1),   (1e-8, 50),   (1e-8, 10),   (1e-8, 50)]  #bounds = []
            p_bounds=[(1e-3, 2),  (2, 10),     (2, 15),     (1e-10, 0.5), (1e-10, 0.5), (1e-10, 0.5),   (1e-8, 50),    (1e-8, 5),   (1e-8, 20)] # plausible bounds [PLB, PUB], more narrow than the bounds


        if self.config['nConfK'] == 4 and self.config['Causal_readout'] == 'Bay':
        ...
        if self.config['Causal_readout'] == 'Bay' and self.config['dec_noise_flag'] in (2, -1):
            ...
            bounds.append((-4, 4))  
            p_bounds.append((-0.5, 1.5))
        if self.config['Causal_readout'] in ['xdiff', 'sdiff']:
    ...
    def perform_BIC(self, xV, xA, sigV, sigA, ntrial, Atrial):
        """ 
        ...
        if self.config['est_var'] == 'MA':
            ...
            return sV_hat, sA_hat, sim_post, s_hat_common, sV_hat_indep, sA_hat_indep, varVA_hat, varV_hat, varA_hat
            ...
            return sV_hat, sA_hat, sig_hat_MS, sim_post
            ...
            return sV_hat, sA_hat, sim_post, post_cdf, post_pdf, bin_est, binSize, nbin, responseLoc_boundaries
        
    def perform_heuristic_inference(self, xV, xA, varV):
    ...
    def run_simulation(self, inputs, trialConds=None, interimOutput=False):
        ...
        if self.config['Causal_readout'] == 'Bay':
            ...
            if self.config['est_var'] == 'MA':
                sV_hat, sA_hat, sim_post, s_hat_common, sV_hat_indep, sA_hat_indep, varVA_hat, varV_hat,  \
                varA_hat = self.perform_BIC(xV, xA, sigV, sigA, ntrial, Atrial)
        
            elif self.config['est_var'] == 'MS':
                sV_hat, sA_hat, sig_hat_MS, sim_post = self.perform_BIC(xV, xA, sigV, sigA, ntrial, Atrial)
        
            elif self.config['est_var'] == 'MAP':      
                sV_hat, sA_hat, sim_post, post_cdf, post_pdf, bin_est, binSize, nbin,   \
                responseLoc_boundaries = self.perform_BIC(xV, xA, sigV, sigA, ntrial, Atrial)
            ...
            if self.config['Causal_readout'] == 'NonBayFixSig':
                ...
                varV = sigV_emp**2
            elif self.config['Causal_readout'] == 'NonBayFleSig':
                varV = sigV**2

            sA_hat, sV_hat, w_c1 = self.perform_heuristic_inference(xV, xA, varV)
            # report causal judgement
        ...
        if self.config['est_var'] == 'MA' and self.config['Causal_readout'] == 'Bay' and self.config['CI_readout'] in ['BayIntFlex','BayIntFix',  'BayEst', 'Entro']:
        ...
        if self.config['CI_readout'] in ['BayIntFlex','BayIntFix'] and self.config['Causal_readout'] == 'Bay':
        ...
        elif self.config['CI_readout'] == 'BayEst' and self.config['Causal_readout'] == 'Bay':
        ...
        elif self.config['CI_readout'] == 'Entro' and self.config['Causal_readout'] == 'Bay':
        ...
        elif self.config['Causal_readout'] == 'NonBayFixSig' or self.config['Causal_readout'] == 'NonBayFleSig':

            resp[:, 1] = self.params.b + self.params.a /w_c1
        ...
        if self.config['Causal_readout'] == 'Bay':
            ...
            # Output: 1 - common source, 2 - separate source
            #resp[:, 2] = torch.where(w_c1>.5, 1, 2)
            resp[:, 3] = torch.where(w_c1>.5, w_c1, 1 - w_c1)
        ...
        if interimOutput:
            return predData, sim_post, sim_post_conf, xA, xV
        else:
