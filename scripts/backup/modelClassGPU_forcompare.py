...
class ConfiModelGPU:
    ...
    def add_decision_noise(self, sim_post, conf):
        ...
        # Gamma sampling: shape = rate = sim_post, scale = 1
        gamma_noise = torch.distributions.Gamma(sim_post, torch.tensor(1.0, device=self.device)).sample()
    ...
    def run_simulation(self, inputs, test, trialConds=None, interimOutput=False):
        '''
        Parameters
        ----------
        ...
        varV = torch.zeros(ntrial, device=self.device)
        varV[trialConds[:, 2] == 1] = self.params.sig_Vrh**2
        varV[trialConds[:, 2] == 2] = self.params.sig_Vrl**2

        varA = torch.tensor(self.params.sig_A**2, device=self.device)
        ...
        # xA = trialConds[:, 0] + self.params.sig_A * torch.randn(ntrial, device=self.device)
        # xV = trialConds[:, 1] + torch.sqrt(varV) * torch.randn(ntrial, device=self.device)
        if test['flag']:
            xA = torch.tensor(test['xA'], dtype=torch.float32, device=self.device)
            xV = torch.tensor(test['xV'], dtype=torch.float32, device=self.device)
        ...
        #resp[:, 0] = torch.normal(shat, self.params.sig_r)
        resp[:, 0] = shat
        ...
        # Add motor noise
        #resp[:, 1] = torch.normal(resp[:, 1], self.params.sig_r)
        ...
        if self.config['Causal_readout'] == 'Bay':
            ...
            if self.config['nConfK'] == 2:
                ...
                resp[:, 3] = 1 / (1 + torch.exp(-sim_c_map * self.params.kC1 + self.params.mC1))
                # alternative: linear
                ...
                resp[com_trial, 3] = 1 / (1 + torch.exp(-sim_post_conf[com_trial, 0] * self.params.kC1 + self.params.mC1))
                resp[~com_trial, 3] = 1 / (1 + torch.exp(-sim_post_conf[~com_trial, 1] * self.params.kC2 + self.params.mC2))
                # alternative: linear
            ...
            if self.config['nConfK'] == 2:
                # symmetrical boundary
                resp[com_trial, 3] = 1 / (1 + torch.exp(d[com_trial] * self.params.kC1 - self.params.mC1))
                resp[~com_trial, 3] = 1 / (1 + torch.exp(-d[~com_trial] * self.params.kC1 + self.params.mC1))
                # alternative: linear
                ...
                # asymmetrical boundary
                resp[com_trial, 3] = 1 / (1 + torch.exp(d[com_trial] * self.params.kC1 - self.params.mC1))
                resp[~com_trial, 3] = 1 / (1 + torch.exp(-d[~com_trial] * self.params.kC2 + self.params.mC2))
                # alternative: linear
                # resp[com_trial, 3] = -d[com_trial] * self.params.kC1 + self.params.mC1
        ...
        noise = torch.randn(resp.shape[0], device=self.device) * self.params.sig_r
