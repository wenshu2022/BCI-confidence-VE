...
def get_cond_n_bin_data(data, bin_data, condition, cix):
    ...
    bin_data : dict
        bin edges and bin indecis from behavuour data.
    ...
    return data[cond_mask, 4:], bin_data[cix]['loc_grid'], bin_data[cix]['ci_grid'], bin_data[cix]['conf_grid'], bin_data[cix]['bin_indices']


def comput_LL(pred_resp, loc_grid, ci_grid, conf_grid, beh_bin_indices, num_sim, eps, detail= False):
    '''
    ...
    conf_grid : bin edges for causal confidence  from behaviour data.
    ...
    conf_binned  = np.digitize(pred_resp[:, 3], bins=conf_grid, right=True) - 1  

    bin_indices = np.column_stack((loc_binned, ci_binned, pred_resp[:, 2].astype(int)-1, conf_binned))
    #compute the occurence of the sim_dat in each bin
    bin_sizes = (len(loc_grid)-1, len(ci_grid)-1, 2, len(conf_grid)-1)
    ...
    valid_mask = (
        ...
        (bin_indices[:, 3] >= 0) & (bin_indices[:, 3] < bin_sizes[3])
    )
...
def func_compute_NLL(model, bin_data, num_sim=10000, eps=1e-2):
    ...
    def neg_log_likelihood(params):
        ...
        for cidx in range(model.conditions.shape[0]):
            ...
            pred_resp, loc_grid, ci_grid, conf_grid, beh_bin_indices= get_cond_n_bin_data(simulated_data, bin_data, c_, cidx)
        
            # Compute LL
            LL += comput_LL(pred_resp, loc_grid, ci_grid, conf_grid, beh_bin_indices, num_sim, eps)
...
def func_compute_NLL_detail(model, bin_data, num_sim=10000, eps=1e-2):
    ...
    def neg_log_likelihood_lst(params):
        ...
        for cidx in range(model.conditions.shape[0]):
            ...
            log_probs,  LL = comput_LL(pred_resp, loc_grid, ci_grid, conf_grid, beh_bin_indices, num_sim, eps, True)
            beh_bins.append(beh_bin_indices)
            # Compute LL
            LL_lst.append(-1 * LL)
            log_probs_lst.append(log_probs)
        # return negative log-likelihood
        return LL_lst, beh_bins, log_probs_lst
